...
namespace UrbanX.Application
{
    public class SunCalculator
    {
        ...
        private readonly NTSPoint origin = new NTSPoint(0, 0, 0);

        /// <summary>
        /// The 3d vector of current sun light.
        /// </summary>
        public Vector2D SunVector { get; }
        ...
        public SunCalculator(double latitude, double height, double longtitude, int year = 2018, int month = 12, int day = 23, int hour = 12, double northAngle = 0d, double timeZone = 8d)
        {
            ...
            SunVector = GetSunVector2D(latitude, longtitude, year, month, day);
        ...
        private Vector2D GetSunVector2D(double latitude, double longtitude, int year, int month, int day)
        {
            ...
            var basePt =Pt3DExten.Add(_selfCenpt,new NTSPoint(0, _selfScale, 0));
            var rotateVec2 = new Vector2D(-(basePt.X+Math.Cos(solAz - angle2North) *Math.Cos(solAlt)), -(basePt.Y*Math.Sin(solAlt)));
            var result = rotateVec2.Normalize();
        ...
        /// <returns></returns>
        private double GetDistanceBasedOnSunVec(double height, Vector2D sunVector)
        {
            var shadowAngle = Math.Abs(sunVector.Angle());
            var distance = height * (1 / Math.Tan(shadowAngle));
            return Math.Abs(distance);

            //Rhino.Geometry.Intersect.Intersection.LinePlane(lightLine, Plane.WorldXY, out double lineValue);
            //var distance = lightLine.PointAt(lineValue).DistanceTo(origin);
        }
    }

    public static class Vec3DExten
    {
        public enum Axis
        {
            XAxis,
        ...
        public static Vector3D GetVecXAxis(Axis axis)
        {
        ...
        public static Vector3D CreateVecFromPt(NTSPoint pt)
        {
            return new Vector3D(pt.X, pt.Y, pt.Z);
        }
    }

    public static class Pt3DExten
    {
        public static NTSPoint CreatePtFromVec(Vector3D vec)
        {
            return new NTSPoint(vec.X, vec.Y, vec.Z);
        }

        public static NTSPoint Add(NTSPoint pt1, NTSPoint pt2)
        {
            return new NTSPoint(pt1.X+pt2.X, pt1.Y + pt2.Y, pt1.Z + pt2.Z);
        }

        public static NTSPoint Substract(NTSPoint pt1, NTSPoint pt2)
        {
            return new NTSPoint(pt1.X - pt2.X, pt1.Y - pt2.Y, pt1.Z - pt2.Z);
        }
    }
